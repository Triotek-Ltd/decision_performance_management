"""Action handler seed for decision_option:select."""
...
ACTION_ID = "select"
ACTION_RULE: dict[str, Any] = {'allowed_in_states': ['draft', 'reviewed', 'selected', 'rejected'], 'transitions_to': None}
...
ACTION_CONTRACT: dict[str, Any] = {'rule': {'allowed_in_states': ['draft', 'reviewed', 'selected', 'rejected'], 'transitions_to': None}, 'requires_action_comment': False, 'requires_reason_for_change': False, 'requires_evidence': False, 'is_disposition_action': False, 'creates_submission_snapshot': False, 'creates_official_copy': False, 'requires_signature': False}

def handle_select(payload: dict, context: dict | None = None) -> dict:
    context = context or {}
